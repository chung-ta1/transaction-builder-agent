...
- **PASSIVE / property-state phrasing is NOT a side signal — never infer a side from it.** "the property is/was sold", "it sold for $X", "sold for 100k", "this place sold", "the home is on the market" describe the *property*, not who the user represented. Bare "sold"/"bought" with no first-person actor (no "I"/"my") carries ZERO buyer-vs-seller information — it only sets `dealType = SALE`. In that case leave `representationType` unset and let the validator ask "Which side are you representing?"; never guess or default a side (a wrong side silently routes the deal down the seller-side listing chain).
