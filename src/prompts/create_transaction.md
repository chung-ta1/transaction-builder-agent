...
- **MLS:** for an off-MLS / exclusive deal, set `location.mlsNumber` to the literal string `"N/A"` — never null/omitted. arrakis hard-rejects a null MLS at submit ("mls number can't be null in Transaction"), so an omitted MLS lets create succeed but blocks submit.
...
- **Representation:** set a side ONLY from an *explicit role phrase* or a *first-person ACTIVE* deal idiom — where the user themselves is the actor. Never infer a side from a passive/property-state description, and never default one.
  ...
  - **PASSIVE / property-state phrasing is NOT a side signal — never infer a side from it.** "the property is/was sold", "it sold for $X", "sold for 100k", "this place sold", "the home is on the market" describe the *property*, not who the user represented. Bare "sold" with no first-person actor carries ZERO buyer-vs-seller information. (It still sets deal type = SALE per the Deal-type rule below — but says nothing about which side.) Same for "bought"/"purchased" without "I/my".
  - **Never guess or default a side.** When the prompt has no explicit role phrase AND no first-person active idiom, leave `representationType` unset so the validator fires its "Which side are you representing?" `AskUserQuestion`. Do NOT pass a fabricated side into the validator to suppress that question — a wrong side silently routes the whole deal (seller-side runs a different listing chain) and is a financial/identity error.
  - When a side IS set from a first-person idiom (rather than the literal words "buyer's/seller's agent"), surface it on the `✓ Representation:` parse-summary line so a misread is catchable.
